    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth PaperProps={{
            ...
            <DialogActions>
                ...
                <Button onClick={handleAdd} variant="contained" disabled={!roleName}>Ajouter</Button>
            </DialogActions>
        </Dialog>
    );
};
...
const DeleteRoleDialog = ({ open, onClose, roles, onDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSelectedRole('');
      if(!errorRole) {onClose()};
      window.location.reload();

    } catch (err) {
      if (err.response?.status === 400) {
        setError("Impossible de supprimer le rôle car il est attribué aux utilisateurs.");
      } else {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth PaperProps={{
      ...
      <DialogActions>
        ...
        <Button onClick={handleDelete} variant="contained" color="error" disabled={loading || !selectedRole}>{loading ? "Suppression..." : "Effacer"}</Button>
      </DialogActions>
    </Dialog>
  );
};
...
const UserDialog = ({ open, onClose, user, roles, onUpdate }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth PaperProps={{
      ...
      <DialogTitle>{`${user.firstName} ${user.familyName.toUpperCase()}`}</DialogTitle>
      <DialogContent>
        <Typography variant="subtitle1" gutterBottom>Roles actuels:</Typography>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          {user.roles.map(role => (
            <Chip key={role} label={role} size="small" sx={{backgroundColor: "var(--design-color)",padding:2,fontSize:"1rem"}}/>
          ))}
        </Box>
        <FormControl fullWidth margin="normal" sx={{backgroundColor: "#333" ,color: "#333",borderRadius: "16px",}}>
        ...
        <FormControl component="fieldset" margin="normal" >
          <RadioGroup
            ...
            onChange={(e) => setAction(e.target.value)}
                sx={{backgroundColor: "#F2EFE7" ,color: "#333",borderRadius: "16px",}}
            
          >
            <FormControlLabel value="add" control={<Radio />} label="Ajouter" disabled={loading}/>
            <FormControlLabel value="remove" control={<Radio />} label="Retirer"disabled={loading} />
          </RadioGroup>
        </FormControl>
      </DialogContent>
      <DialogActions>
        <Button onClick={onClose} variant='contained' color='error'>Fermer</Button>
        <Button onClick={handleApply} variant="contained" style={{backgroundColor:"var(--primary-color)"}}>{loading ? "Traitement..." : "Appliquer"}</Button>
      </DialogActions>
    </Dialog>
  );
};
...
const RoleGroupConfig = () => {
  ...
  const handleUpdateUser = (user, role, action) => {
    setUsers(prevUsers => prevUsers.map(u => {
      if (u.id === user.id) {
        if (action === 'add') {
          return { ...u, roles: u.roles.includes(role) ? u.roles : [...u.roles, role] };
        } else if (action === 'remove') {
  ...
  return (
    <Box sx={{ padding: 2, margin: 'auto' }}>
      ...
      <Box sx={{ border: '1px solid #1D1616',borderRadius:"15px", padding: 2, m: 2,mb: 10,width: "95%" }}>
        <Box sx={{ display: 'flex', flexDirection: 'row', overflowX: 'auto', whiteSpace: 'nowrap', gap: 2 }}>
          {roleNames.length >0 ? roleNames.map(role => (
            <Tooltip
              key={role}
              title={users.filter(user => user.roles.includes(role))
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, m: 2 }}>
        ...
        <Button
          variant="contained"
          sx={{ backgroundColor: 'var(--design-color)', color: 'white', '&:hover': { backgroundColor: '#333' } }}
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
          {users.map(user => (
            <Chip
              key={user.id}
              sx={{fontSize: "1.2rem"}}
              label={`${user.firstName} ${user.familyName.toUpperCase()}`}
