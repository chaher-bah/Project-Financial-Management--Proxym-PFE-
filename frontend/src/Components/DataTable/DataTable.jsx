...
const enhancedColumns = [
    {
      ...
      renderCell: (params) => {
        ...
        return (
          <IconButton 
            size="small" 
            onClick={() => toggleParent(params.row.id)}
          >
...
rows.forEach(row => {
  ...
  return (
    <Container className="data-table-container" style={{margin:1,boxSizing:"unset",maxWidth:"100%"}}>
      <Box className="data-table-header" sx={{
        ...
        <Typography variant="h4" component="h1" gutterBottom sx={{m:2.5,fontWeight:"600"}}>
          {title}
        </Typography>
        {backPath && (
